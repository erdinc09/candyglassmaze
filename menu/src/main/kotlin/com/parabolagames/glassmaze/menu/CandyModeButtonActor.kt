...
internal class CandyModeButtonActor @Inject constructor(
    ...
    private val candyAtlas = Assets.CANDY14_ATLAS
    ...
    init {
        ...
        ringActor = TableActor().apply {
            loadTexture(assets.getTexture(Assets.CANDY_RING))
            scaleBy(0.45f)
        }
        resetRingActor()
        addActor(ringActor)
        ...
        candyInsideGlass = TableActor().apply {
            ...
            setSize(this@CandyModeButtonActor.width / 1.5f, this@CandyModeButtonActor.height / 1.5f)
            setOrigin(width / 2, height / 2)
            setPosition(this@CandyModeButtonActor.width / 2 - width / 2, this@CandyModeButtonActor.height / 2 - height / 2)
        }.also { addActor(it) }
    ...
    private fun resetRingActor() {
        with(ringActor) {
            setOrigin(
                    Constants.MENU_BUTTON_WIDTH_SMALL / 2, Constants.MENU_BUTTON_WIDTH_SMALL / 2)
            setSize(Constants.MENU_BUTTON_WIDTH_SMALL, Constants.MENU_BUTTON_WIDTH_SMALL)
            setPosition(0f, 0f)
            addAction(forever(Actions.rotateBy(-360f, 15f)))
    ...
    override fun act(dt: Float) {
        if (!disabled && clickListener.isPressed) {
            controller.get().candyModeButtonPressed()
            ...
            ringActor.addAction(
                    sequence(
                            Actions.delay(2f),
                            Actions.run { controller.get().setCandyModeScreenVisible() }))

            candyThrown.apply {
                setAnimationAlignedFromActor(candyInsideGlass)
                reInit(
                        this@CandyModeButtonActor.x + this@CandyModeButtonActor.width / 2 - CANDY_SIZE / 2,
                        this@CandyModeButtonActor.y + this@CandyModeButtonActor.height / 2 - CANDY_SIZE / 2, world)
                applyForce(-5f, 16f)
            }.also {
    ...
    companion object {
        private const val TAG = "CandyModeButtonActor"
        private const val CANDY_SIZE = 0.40f
    }
}